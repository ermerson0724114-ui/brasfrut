      ...
      <div className="bg-green-900 text-white px-4 pt-6 pb-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Users size={22} />
            <div>
              <h1 className="text-xl font-extrabold">Funcionários</h1>
              <p className="text-green-200 text-xs">{employees.length} cadastrado(s)</p>
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={() => setModal("csv")} className="w-9 h-9 bg-green-800 rounded-xl flex items-center justify-center" data-testid="button-import-csv"><Upload size={16} /></button>
            <button onClick={() => { setForm(emptyForm); setEditId(null); setModal("add"); }} className="w-9 h-9 bg-white/20 rounded-xl flex items-center justify-center" data-testid="button-add-employee"><Plus size={18} /></button>
          </div>
        </div>
